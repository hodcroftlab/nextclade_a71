...
clades_with_high_proportion_threshold = [ # TODO: update as needed
    "A",
    "B",
    "C",
    "D",
...
df = pd.read_csv(
    "test_out/nextclade.tsv",
    sep="\t",
    usecols=["clade", "substitutions"],
    parse_dates=False,
    dtype={"clade": str, "substitutions": str},
).dropna()
df.rename(columns={"clade": "Nextstrain_clade"}, inplace=True)
...
mutations["short_clade"] = mutations["Nextstrain_clade"].apply(lambda x: x[:8])
mutations
...
with open("results/virus_properties.json", "w") as f_out:
